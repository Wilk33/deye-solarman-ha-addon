# SolarMan Diagnostics 2.0.1 - architektura jednego dodatku

## Stan bieżący

Wersja 2.0.1 ma jeden instalowalny katalog `deye-solarman-diagnostics`, jeden proces runtime i jeden klient MQTT. Ten proces może obsługiwać SolarMan TCP, Modbus RTU przez USB-RS485 albo oba transporty równocześnie.
...
Indeks katalogu ma revision `2.0.1`. Obraz przypina `pymodbus==3.14.0` dla adaptera RTU.
...
Pusty `default_profile` oznacza, że nowa instalacja nie włącza ukrytego zestawu encji.
...
`config.yaml` ma `version: "2.0.1"`, `uart: true`, sekcje `solarman` i `rs485` oraz kompletne tłumaczenia PL/EN. Pola tożsamości falownika, profili, diagnostyki i skanowania są na najwyższym poziomie, dzięki czemu Home Assistant pokazuje je stale i zapisuje przełączniki jako bezpośrednie wartości konfiguracji.
